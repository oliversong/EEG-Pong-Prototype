...
class CytonBinParser(metaclass=ABCMeta):
    ...
    def start_collect(self, buffer_size=2**8, clear=True, raw=None):
        ...
        self.thread_data_collect = threading.Thread(target=self.collect_data, args=(buffer_size, ))
        self.thread_data_collect.start()

        if not self.RAW:
            # Thread for deserialize data
            self.thread_deserialize = threading.Thread(target=self.deserialize_data)
    ...
    def pack_data(self, milliseconds=100):
        ...
        self.thread_pack = threading.Thread(target=self.pack_data_samples, args=(milliseconds, ))
    ...
    def pack_data_samples(self, milliseconds):
        ...
        while self.READING or not self._data_eeg.empty():
            ...
            pack = [self._data_eeg.get() for i in range(5)]
            while pack[-1][4] - pack[0][4] < timedelta(milliseconds=milliseconds):
                pack.extend([self._data_eeg.get() for i in range(5)])
                # pack.append(self._data_eeg.get())
                # pack.append(self._data_eeg.get())

            pack_eeg = np.array([p[1] for p in pack])
            pack_aux = np.array([p[2] for p in pack])
            pack_datetime = np.array([p[4].timestamp() for p in pack])
            self._data_packs.put([pack_eeg.T, pack_aux.T, pack_datetime.T])

            # if self._data_eeg.qsize() > samples:

                ##pack_buffer = self._data_packs.qsize()
                # if pack_buffer:
                    # logging.warning(f'Pack buffer: {pack_buffer}')

                #pack = [self._data_eeg.get() for i in range(samples)]

                #pack_eeg = np.array([p[1] for p in pack])
                #pack_aux = np.array([p[2] for p in pack])
                #pack_datetime = np.array([p[4] for p in pack])
                #self._data_packs.put([pack_eeg.T, pack_aux.T, pack_datetime.T])
    ...
    def deserialize_data(self):
        ...
        while self.READING or not self._data_serial.empty():
            ...
            if hex(stop_byte)[-2].lower() != 'c':
                self._no_valid += 1
                logging.warning(f'Invalid stop byte {hex(stop_byte)} for sample {sample}, {100*(self._no_valid/self._valid):.3f}% loss')
                continue
            else:
                self._valid += 1
