...
class Cyton(CytonBinParser):
    ...
    the board, this dirver use a lot of "defined there" instructions.
    ...
    def gain(self):
        ...
        # return self.VREF / (self.gain * ((2 ** 23) - 1))
    ...
    # ----------------------------------------------------------------------
    def send_marker(self, marker, burst=10):
        ...
        self.thread_marker = threading.Thread(target=thread_marker, args=(marker, ))
    ...
    def daisy(self):
        ...
        if ('no daisy to attach' in response.decode()) or ('8' in response.decode()):
            # 8 channels
            self.leadoff_impedance(range(8), self.TEST_SIGNAL_NOT_APPLIED, self.TEST_SIGNAL_APPLIED)
            # time.sleep(0.2)
            self.activate_channel(range(8))
            return False
        else:
            # 16 channels
            self.leadoff_impedance(range(16), self.TEST_SIGNAL_NOT_APPLIED, self.TEST_SIGNAL_APPLIED)
            # time.sleep(0.2)
            self.activate_channel(range(16))
            return True
...
class CytonRFDuino(Cyton):
    ...
    # ----------------------------------------------------------------------
    def __init__(self, port=None, file=None, timeout=0.5, write_timeout=0.1, sample_rate=Cyton.SAMPLE_RATE_250HZ):
...
########################################################################
class CythonWiFi(Cyton):
    """Not implemented yet."""

    # ----------------------------------------------------------------------
    def __init__(self, ip_address, local_ip_address=None, sample_rate=Cyton.SAMPLE_RATE_16KHZ):
        """"""
        self.sample_rate_ = self.SAMPLE_RATE_VALUE[sample_rate]
    ...
    def create_server(self):
        """"""
        self.local_wifi_server = WiFiShieldServer(self.local_ip_address, self._data_serial)
    ...
    def connect(self):
        ...
        response = requests.get("http://%s/board" % self.ip_address)
    ...
    def star_loop(self):
        """"""
        import threading
        self.th_loop = threading.Thread(target=asyncore.loop, args=(), )
        self.th_loop.start()
    ...
    # ----------------------------------------------------------------------
    def start_collect(self, *args, **kwargs):
        """"""
    ...
    def stop_collect(self, *args, **kwargs):
        ...
        # self.stop_loop()
        # self.stop_streaming()
        super().stop_collect(*args, **kwargs)
    ...
    def collect_data(self, size=2**8):
        ...
        # while self.READING:
            # while not self.read_queue.empty():
                # [self._data_serial.put(data) for data in self.read_queue.get()]
...
class WiFiShieldServer(asyncore.dispatcher):
    ...
    def __init__(self, host, data_queue):
        ...
        # self.device_handle_read = handle_read
    ...
    def handle_accept(self):
        ...
        if pair is not None:
            ...
            self.handler.handle_read = self.handle_read
            # asyncore.loop()
            # self.device_handle_read = self.handler.recv
            # self.

    # ----------------------------------------------------------------------
    def handle_read(self):
        """"""
        data = self.handler.recv(3000)
        [self.data.put(d) for d in data]
        # self.i += len(data)
        # print(self.i / 33)
        # self.data.put(data)
        # self.device_handle_read(data=data)
